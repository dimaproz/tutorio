...
describe('unconfirmed attendance (L-72, L-74)', () => {
  ...
  it('leaves out a confirmed, running, cancelled or individual lesson', () => {
    ...
    expect(needsAttendance({ ...group, startsAtUtc: new Date('2026-09-26T09:30:00.000Z') }, now)).toBe(false);
...
describe('packages that would expire unused (L-84)', () => {
  it('flags a live package whose window closes within three days', () => {
    expect(expiresUnused({ remainingCredits: 3, expiresAt: new Date(now.getTime() + days(3)) }, now)).toBe(
      true,
    );
  });
  ...
  it('leaves out a later end, a used-up, an expired or an open package', () => {
    ...
    expect(expiresUnused({ remainingCredits: 0, expiresAt: new Date(now.getTime() + days(1)) }, now)).toBe(
      false,
    );
    expect(expiresUnused({ remainingCredits: 2, expiresAt: new Date(now.getTime() - 1) }, now)).toBe(false);
...
describe('pauses to look at (L-103)', () => {
  it('flags a running pause that ends within three days', () => {
    const pause = { startsAt: new Date(now.getTime() - days(10)), endsAt: new Date(now.getTime() + days(2)) };
    expect(pauseAttention(pause, now)).toBe('RETURNING');
  });

  it('flags an open pause after thirty days', () => {
    expect(pauseAttention({ startsAt: new Date(now.getTime() - days(45)), endsAt: null }, now)).toBe(
      'OPEN_LONG',
    );
    expect(pauseAttention({ startsAt: new Date(now.getTime() - days(29)), endsAt: null }, now)).toBeNull();
  });

  it('leaves out a later return, a planned or an ended pause', () => {
    const running = { startsAt: new Date(now.getTime() - days(1)) };
    expect(pauseAttention({ ...running, endsAt: new Date(now.getTime() + days(4)) }, now)).toBeNull();
    expect(
      pauseAttention({ startsAt: new Date(now.getTime() + days(1)), endsAt: new Date(now.getTime() + days(2)) }, now),
    ).toBeNull();
    expect(
      pauseAttention({ ...running, endsAt: new Date(now.getTime() + days(2)), endedAt: new Date(now.getTime() - 1) }, now),
    ).toBeNull();
  });
});
...
describe('the seven-day forecast', () => {
  it('expects a renewal when the credits or the window run out within seven days', () => {
    ...
    expect(expectsRenewal({ creditsLeft: 6, expiresAt: new Date(now.getTime() + days(5)), upcomingLessons: 1 }, now)).toBe(
      true,
    );
  });

  it('expects nothing while the credits outlast the week or no lesson comes', () => {
    expect(expectsRenewal({ creditsLeft: 3, expiresAt: null, upcomingLessons: 2 }, now)).toBe(false);
    expect(expectsRenewal({ creditsLeft: 1, expiresAt: null, upcomingLessons: 0 }, now)).toBe(false);
  });
...
describe('money received', () => {
  ...
  it('sums this month and today per currency, refunds taken off', () => {
    const totals = receivedTotals(
      [
        { amountMinor: 320000, currency: 'UAH', status: 'PAID', paidAt: new Date('2026-09-26T07:00:00.000Z') },
        { amountMinor: 1000000, currency: 'UAH', status: 'PAID', paidAt: new Date('2026-09-02T07:00:00.000Z') },
        { amountMinor: 50000, currency: 'UAH', status: 'REFUNDED', paidAt: new Date('2026-09-10T07:00:00.000Z') },
        { amountMinor: 20000, currency: 'PLN', status: 'PAID', paidAt: new Date('2026-09-05T07:00:00.000Z') },
        // Before the studio's month began.
        { amountMinor: 90000, currency: 'UAH', status: 'PAID', paidAt: new Date('2026-08-31T20:59:00.000Z') },
      ],
      periods,
    );
