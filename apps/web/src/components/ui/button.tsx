...
const buttonVariants = cva(
  "group/button relative inline-flex shrink-0 items-center justify-center rounded-pill border border-transparent bg-clip-padding font-medium whitespace-nowrap transition-all outline-none select-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/30 active:not-aria-[haspopup]:translate-y-px disabled:pointer-events-none disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-3 aria-invalid:ring-destructive/20 dark:aria-invalid:border-destructive/50 dark:aria-invalid:ring-destructive/40 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
  {
    variants: {
      variant: {
        ...
        outline:
          'border-border bg-card text-foreground hover:bg-surface-hover aria-expanded:bg-surface-hover dark:bg-transparent dark:hover:bg-input/30',
        white: 'bg-card text-foreground hover:bg-surface-hover aria-expanded:bg-surface-hover',
        soft: 'bg-brand-soft font-semibold text-brand-soft-foreground hover:bg-brand-soft/85',
        'dark-outline': 'border-ink-line bg-transparent text-ink-foreground hover:bg-ink-soft',
        secondary:
...
function Button({
  ...
  // Slot clones a single child, so a composed button owns its own decoration
  // and must never be handed an array of children.
  if (asChild) {
    return (
      <Slot.Root
        data-slot="button"
