...
export function ProfileHero({
  ...
  return (
    <Card
      ...
      className={cn('relative h-80 flex-row items-center gap-7 p-8', className)}
    >
      {glyph ? (
        ...
        <span
          ...
          className="pointer-events-none absolute -right-[18px] -bottom-[86px] text-[300px] leading-[300px] font-bold tracking-[-0.06em] text-tint-indigo-glyph select-none after:content-[var(--profile-glyph)]"
        />
      ) : null}
      ...
      <div className="relative flex min-w-0 flex-col gap-3.5">
        ...
        <h1 className="text-[56px] leading-[56px] font-semibold tracking-[-0.04em]">{name}</h1>
        {metaItems.length > 0 ? (
