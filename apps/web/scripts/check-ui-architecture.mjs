...
const primitiveNames = new Set([
  'accordion', 'alert-dialog', 'alert', 'aspect-ratio', 'avatar', 'badge', 'breadcrumb', 'bubble',
  'button', 'calendar', 'card', 'chart', 'checkbox', 'collapsible', 'command', 'context-menu',
  'dialog', 'direction', 'drawer', 'dropdown-menu', 'empty', 'field', 'hover-card', 'input-group',
...
const duplicatePrimitiveAllowlist = new Set([
  'lib/pagination.ts',
]);
...
const removedUtilityPattern = /\bfont-heading\b|\b(?:bg|text|border|ring)-light(?:-|[a-z])/;
...
for (const file of files) {
  ...
  if (
    ...
    primitiveNames.has(relativePath.replace(/\.tsx?$/, '').split('/').at(-1))
  ) {
    violations.push(`${relativePath}: duplicate primitive filename outside components/ui`);
  }

  if (!rawColorAllowlist.has(relativePath)) {
    const rawColor = /#[0-9a-fA-F]{3,8}\b|\b(?:bg|text|border|ring|fill|stroke)-(?:red|orange|amber|yellow|lime|green|emerald|teal|cyan|sky|blue|indigo|violet|purple|fuchsia|pink|rose|slate|gray|zinc|neutral|stone)-\d{2,3}\b|\b(?:oklch|oklab|hsl|hsla|rgb|rgba)\(\s*(?!from(?:\b|_))/;
