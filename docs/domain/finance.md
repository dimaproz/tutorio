...
| Concern      | Contract                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Purpose      | Prepaid lesson credits for one direction (`enrollmentId`; `studentId` is its student). Kinds (`sizingMode`, L-80): `FIXED_COUNT` (N lessons, optional `expiresAt`), `BY_PERIOD` (the lessons the direction's schedule has from `validFrom` to `endDate`, or the tutor's count), `BY_PERIOD_WEEKLY` (`lessonsPerWeek` × weeks in the window, rounded to whole weeks). Priced per lesson or as a total; the purchase-time price is an immutable snapshot.                                                                                                                                                                                                                                           |
| Ownership    | Workspace-scoped. A group's packages are its members' packages for the group (`GET /packages?groupId=`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| Create       | `POST /packages` with `studentId` and, for a group membership, `groupId` (else the student's lessons with `teacherId` or their only teacher; a new direction is created silently, L-2). `POST /packages/preview` returns the same plan without writing. `POST /packages/members` sells one spec to each selected member of a group in one transaction (L-86). A sale writes the package and its `purchase` entry, switches a pay-per-lesson direction to packages and covers its debt; the current form may still add a first payment and the direction's schedule, the member sale never does (L-87).                                                                                            |
| Credits      | Remaining = credit entries − active charges it pays for; consumed = those charges. A package pays only for lessons in `[validFrom, expiresAt)`. `GET /packages/:id/ledger` lists the entries and one `lesson` row per paid lesson.                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| Operations   | `adjust` appends a signed `manual_adjustment` (a positive one covers debt). `extend` moves `expiresAt` later (not for a package without an end) and covers debt in the new window. `transfer` moves unused credits to another direction of the same student in the same currency: a new count package there worth `floor(credits × source price / target rate)` lessons, owing nothing (`transfer_out`/`transfer_in` entries, `transferredFromPackageId`), the remainder reported (`409 NOT_ENOUGH_CREDITS`, `INVALID_TRANSFER_TARGET`). `refund` takes credits back (`refund` entry) and records the money returned as a `REFUNDED` payment, at most what was received (`409 REFUND_TOO_LARGE`). |
| Archive      | Soft delete: pays for no new lesson; its credits, charges and payments stay. Restore is not supported.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| Current gaps | The low-credit warning is phase 7; pause extensions are phase 6.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |

## LessonCreditEntry

| Concern   | Contract                                                                                                                                     |
| --------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| Purpose   | Append-only record of the credits a package was granted and moved: `purchase`, `manual_adjustment`, `transfer_out`, `transfer_in`, `refund`. |
...
| Refund       | A package refund is a `REFUNDED` payment row (positive amount). The package reports `paidMinor` net of refunds and `refundedMinor`; its status compares net paid with the price net of refunds.                                                                     |
| Current gaps | Future `paidAt` is accepted; a payment itself cannot be corrected.                                                                                                                                                                                                  |
