...
| Archive           | Archive (`status = ARCHIVED`, `archivedAt`) may hand the future scheduled lessons, active schedules and led groups to another active teacher after a teacher-clash check (S09); individual directions and history keep the archived teacher. The owner turns their own teaching off with the same command. Soft delete is not offered in the UI. |
