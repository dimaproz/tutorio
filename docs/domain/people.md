...
| Business lifecycle | `ACTIVE -> ON_HOLD -> ARCHIVED`; status controls operational visibility, not historical deletion. `ON_HOLD` is a running whole-student [pause](./learning-operations.md#pause): setting it opens one, setting `ACTIVE` ends it.                                                                                                                                  |
...
| Archive           | Archive (`status = ARCHIVED`, `archivedAt`) may hand the future scheduled lessons, active schedules, led groups and live one-to-one directions (with their billing; a student who already has a direction with the new teacher keeps this one) to another active teacher after a teacher-clash check (S09); history keeps the archived teacher. The owner turns their own teaching off with the same command, except a solo tutor (`SOLO_OWNER_MUST_TEACH`). Soft delete is not offered in the UI. |
