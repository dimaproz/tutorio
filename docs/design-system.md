...
Last verified: 2026-09-25 (S04, S05, S06, S07).
...
| Entity input and reusable value controls | `EntityPicker`, `AvatarPicker`, `TimezoneCombobox`, `WeekdayPicker` — `@/components/shared/*`                                                                                                                                                                           | controlled values and localized labels; no API calls; `EntityPicker` `appearance` pill/field with `icon`, rich rows when an option has `description`, `badges` or a `trail` (the lesson form's teacher list: «основний», «Зайнятий о 17:00»), `locked` (a lock instead of the chevrons), option `section` headings, `media` (a group tile) and `disabled` (listed, not pickable — a direction in another currency), `onSearchChange` (server-side search), `footer` note; `WeekdayPicker` `appearance` default/pills | Feature forms                                                            | `Shared/Form/EntityPicker` (`teacherField` shows the lesson form's teacher field), `Shared/Form/WeekdayPicker`, `Shared/Form/OptionList`, `Shared/Form/AvatarPicker` |
...
| Band window                              | `BandWindow`, `BandWindowLayout` — `@/components/shared/band-window`                                                                                                                                                                                                    | the dialog of a window headed by the indigo band: `size` md (640, the lesson windows)/lg (900, the package sale)/compact (520); phones a full-screen sheet; the layout keeps the band and footer put and scrolls the body (on phones band and body scroll together)                                                                                                                                                                                                                                                  | Lesson windows, package sale                                             | `Lessons/Screens/*`, `Packages/Screens/Sale`                                                                                                                         |
...
| Banners, empty states and set-up         | `Notice`, `EmptyState`, `SetupChecklist` — `@/components/shared/*`                                                                                                                                                                                                      | `Notice` tone danger/warning/info/success with action (default glyphs: alert, triangle, info, check; a pause or archive banner passes its own `icon`; `actionPlacement="stacked"` puts a banner's actions under its text on phones), `appearance` banner/callout (callout: white icon tile, tone-coloured title, adds indigo/neutral — the explanation inside decision dialogs); icon or media, dashed frame, dotted pattern                                                                                         | Feature screens                                                          | `Shared/Feedback/*`, `Shared/Cards/*`                                                                                                                                |
...
| Schedule dialogs (feature)               | `ScheduleCreateDialog`, `ScheduleChangeDialog`, `ScheduleStopDialog`, `ScheduleHorizonDialog`, `SlotChips` — `@/features/lessons`                                                                                                                                       | new: the lesson form band and `WeeklyBlock` (`between` slot, optional dates) with length and horizon, the check with `DateCards` and `ConflictPairs` (`heading`/`explanation` overrides, e.g. a pause return), force after conflicts, the «already exists» callout; change: `CurrentRule`, then `RuleChange`, `CountTiles`, the pairs and the impact list; stop and horizon with their previews                                                                                                                      | Schedules page; student profile and group page next                      | `Schedules/Screens/Schedules`                                                                                                                                        |
...
| Direction pass art                       | `TicketArt`, `ReceiptArt`, `CoinsArt`, `PausedArt` — `@/components/shared/pass-art`                                                                                                                                                                                     | inline SVG drawn in `currentColor` (the caller's tone colours it), `className`; no currency glyph                                                                                                                                                                                                                                                                                                                                                                                                                    | Student profile passes, package tickets (S07); the group page (S08) next | `Students/Screens/Learning`                                                                                                                                          |
| Student learning and billing (feature)   | `DirectionPass`, `PassStub`, `DirectionMenu`, `StudentLearningBlock`, `PauseBanner`, `StudentPackagesTab`, `StudentPaymentsTab`, the payment, direction settings, pause and pause end dialogs — `@/features/students` internal `ui/learning/*`                          | the pass: stub by state (package/low/debt/advance/clear/empty/paused) with credit dots or money, a perforation, the body with the head row, three facts and the action; the block header with the count and currency chips; the ⋯ as a dropdown or a phone sheet; the dialogs as `AdaptiveDialog` with `ImpactList` previews and `ConflictPairs` on a return                                                                                                                                                         | Student profile                                                          | `Students/Screens/Learning`                                                                                                                                          |
| Package ticket, sale and list (feature)  | `PackageTicket`, `CreditDots`, `PaymentRing`, `TicketWindow`, the sale band, fields and tickets, the operation dialogs and the list cells — `@/features/packages` internal `ui/*`; public `PackageSaleDialog`, `PackageTicketModal`, `usePackageTicket`, `PackagesPage` | the vertical ticket: a 250px stub in the state's tint (indigo new/active, grey used, warning expired) with the count, credit dots and art, a dashed tear line with two masked notches, the caller's body; dots left/added/used (`dashed` on tickets, `quiet` in lists, bars up to 4); the window is a 580px modal or a bottom sheet                                                                                                                                                                                  | Student profile, «Пакети» page                                           | `Packages/Screens/*`                                                                                                                                                 |
