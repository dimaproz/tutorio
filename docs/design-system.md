...
| Entity input and reusable value controls | `EntityPicker`, `AvatarPicker`, `TimezoneCombobox`, `WeekdayPicker` — `@/components/shared/*`                                                                              | controlled values and localized labels; no API calls; `EntityPicker` `appearance` pill/field with `icon`, rich rows when an option has `description`, `badges` or a `trail` (the lesson form's teacher list: «основний», «Зайнятий о 17:00»), `locked` (a lock instead of the chevrons); `WeekdayPicker` `appearance` default/pills                                                                                                                                                          | Feature forms                                    | `Shared/Form/EntityPicker` (`teacherField` shows the lesson form's teacher field), `Shared/Form/WeekdayPicker`, `Shared/Form/OptionList`, `Shared/Form/AvatarPicker` |
...
| Adaptive decision dialog                 | `AdaptiveDialog` — `@/components/shared/adaptive-dialog`                                                                                                                   | Dialog on desktop, Drawer on phones; icon tile, `eyebrow`, title, description, scrolling body, primary/secondary, `tertiary` (footer start; last on the sheet), `closeLabel` (desktop ✕), `initialFocus`, `size` md/lg                                                                                                                                                                                                                                                                       | Feature confirmations with fields                | `Shared/Dialogs/AdaptiveDialog`                                                                                                                                      |
