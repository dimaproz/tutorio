...
The Exact Credit Compensation migration (2026-08-26) is deployed everywhere;
its legacy-data verifier was retired with Work Packet 6.4 phase 3, whose
billing migration replaces the lesson debits it checked.
...
`20260927120000_billing_core` (phase 3) replaces lesson credit debits with
`lesson_charges`: it drops `MONTHLY` billing (existing rows become
`PER_LESSON`), deletes lesson debit entries, attaches each student package to
the student's individual direction, removes group packages and participant
shares (their payments stay, without a package) and drops the lesson and
