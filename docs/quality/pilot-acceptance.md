...
| ------------------ | -------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Static pipeline    | Root lint, typecheck, unit tests, and build                                            | Pass — `pnpm generate`, `pnpm lint`, `pnpm typecheck`, `pnpm test`, and `pnpm build` on 2026-09-07; unit totals: domain 92, validation 46, API 109, web 109                                                                                                         |
| API end-to-end     | Isolated PostgreSQL run using the same command and migrations as CI                    | Pass — PostgreSQL 17 isolated database, all 19 migrations, 5 suites / 70 tests on 2026-09-07                                                                                                                                                                        |
| Migration upgrade  | Legacy pre-migration records, forward migration, normalized rows, and restore          | Pass — `apps/api/scripts/verify-finance-migration-upgrade.ts`; separate isolated PostgreSQL 17 database proves application-level fixed-count and legacy `BY_PERIOD` exact compensation, conflict rejection, idempotency, audit metadata, and rollback on 2026-09-07 |
