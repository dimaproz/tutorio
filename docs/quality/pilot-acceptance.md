...
| Sell an individual fixed package and record partial/full payment            | Relationship, currency, idempotency, ledger, and audit assertions   | Fail — ownership invariant is missing                   |
...
| ------------------ | -------------------------------------------------------------------------------------- | --------------------------------------- |
| Static pipeline    | Root lint, typecheck, unit tests, and build                                            | Pass at `3d985d5`                       |
| API end-to-end     | Isolated PostgreSQL run using the same command and migrations as CI                    | Not run locally                         |
| Generated contract | OpenAPI generation is reproducible and web usage policy is resolved                    | Not run                                 |
