...
7. **Student status includes an explicit "on hold" (`канікули`) state**, separate from deletion: hides the student from active lists without losing data, one click to restore. `Student.status: ACTIVE | ON_HOLD | ARCHIVED` — **superseded during the redesign session (2026-07-23)**: `Student` and `Parent` no longer soft-delete. Deleting either is now permanent (hard delete, cascades to `StudentParent` links and — for students — `Enrollment` rows, all in one transaction); there is no trash and no restore endpoint for these two entities. `ARCHIVED` was added as the non-destructive alternative the delete-confirmation dialog offers instead ("archive" vs. "delete permanently"), distinct from `ON_HOLD`. `Group` and `Enrollment` are unaffected and keep the original soft-delete/restore pattern.
...
All business tables have `workspaceId` + composite indexes; `deletedAt` for soft delete — **except `Student` and `Parent`, which switched to hard delete during the redesign session (2026-07-23)**; see decision #7 above.
...
**Stage 3.6. Teachers (staff directory)** — inserted 2026-07-24
