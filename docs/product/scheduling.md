...
| 5 — Package kinds and operations | Implemented 2026-09-24 (API; the sale form of S07 replaced the legacy schedule and first-payment inputs on 2026-09-25) | L-80 (by count, by period from the direction's schedule — editable, by period X a week; per-lesson or total price; a period window), L-84 (extend), L-85 (transfer by price, rounded down, remainder shown; refund of credits and money), L-86, L-87 for the member sale                                                                                                                                                                                                   |
| 6 — Pause                        | Implemented 2026-09-24 (API; the student "on a break" status runs on it)                                                   | L-73 (a paused member is not marked or charged), L-100 (one pause at a time for the same lessons), L-101, L-102 (packages valid at the start; an early end takes the unused part back), L-103 (return: lessons from then on come back, checked for teacher and student conflicts unless forced, or brought back without the overlapping ones — `skipConflicts`, S06; status synced every 10 minutes), L-104; S06 added the create and end previews and `PATCH /pauses/:id` |
| 7 — Read APIs                    | Implemented 2026-09-24 (API; the screens follow the owner's mockups)                                                       | Pages: the Lessons list (`GET /lessons/list`: period, teacher, student, group, status; quick filters unpaid, cancelled, no-show, needs a makeup, with counts), the lesson side panel (`GET /lessons/:id`: charges with their paid state, makeup link, schedule, history), Schedules (`GET /schedules`), the student billing block (`GET /students/:id/billing`); L-82 and L-120 (credit warnings by the studio threshold, `GET /billing/warnings`)                         |
